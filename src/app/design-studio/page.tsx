  ...
  return (
    <div style={{ padding: '140px 0 6rem', minHeight: '100vh', background: 'var(--midnight)' }}>
      <div className="container">
        ...
        <h1 className="heading-xl" style={{ color: '#F0E8D5', margin: '1rem 0 1.25rem' }}>Plan Your Space With Interactive Tools</h1>

        <div style={{ display: 'grid', gap: '1rem', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))' }}>
          {tools.map((tool) => (
            <article key={tool.title} style={{ background: '#1A1714', border: '1px solid rgba(201,168,76,0.18)', borderRadius: 14, padding: '1.1rem', display: 'grid', gap: '0.9rem' }}>
              <div>
                <h2 className="heading-md" style={{ color: '#F0E8D5', fontSize: '1.22rem', marginBottom: '0.35rem' }}>{tool.title}</h2>
                <p style={{ color: '#9A9080', fontSize: '0.92rem', lineHeight: 1.7 }}>{tool.desc}</p>
              </div>
